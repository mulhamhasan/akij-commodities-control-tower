...
import os, io, json, re, urllib.request, urllib.parse
import openpyxl

REGISTER_ID = "1me7vjHVAMV6Wazw4GnIzoPvvb3B_Rhz1CyDUY9FGLh4"
...
def build():
    ...
    # meetings union across sheets
    nums = set()
    for ws in wb.worksheets:
        t = ws.title.lower()
        if not (("supply" in t) or ("demand" in t) or ("finance" in t)):
            continue
        for r in range(1, 11):
    ...
    meeting_nums = sorted(nums)
    meetings = []
    for i, n in enumerate(meeting_nums):
        ord_ = "st" if n == 1 else "nd" if n == 2 else "rd" if n == 3 else "th"
        meetings.append({"n": n, "label": "%d%s Meeting" % (n, ord_), "status": "latest" if i == len(meeting_nums) - 1 else "past"})

    js = ("// Auto-generated from \"Source File\" register.\n"
